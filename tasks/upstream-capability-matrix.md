...
| [DramaClaw](https://github.com/dramaclaw/dramaclaw) | `30efddc` | Novel/story graph; character/scene/prop/voice asset library; episode planning; script review loops; grid/first-frame generation and candidate pool; TTS; assembly/export; infinite canvas; spatial director world/3GS; director assistant; style templates; task center/resume. | Clean-room behavioral parity behind original interfaces. Do not copy service code. | README declares Elastic License 2.0; reviewed Git tree did not expose a root LICENSE blob. ELv2 restricts offering the software as a managed service. |
| [InstantVideo](https://github.com/briefness/InstantVideo) | `2f7fe80` | Compiled production plan; action contract; strictly sequential generation; accepted-tail handoff; technical and temporal semantic QA; idempotent recovery/budgets; normalization, transitions, BGM, TTS, LUT, subtitles, and multi-platform export; shared Web/CLI core. | Adapt MIT architecture patterns; implement provider-neutral version. | MIT. |
| [video-shotcraft](https://github.com/Vincentwei1021/video-shotcraft) | `41ee360` | Remotion product-video pipeline; current README advertises 152 shot recipe cards, 209 style/motion previews, and 149 SFX across 16 categories; 2.5D page camera; typography, transitions, beat sync, BGM/SFX, deterministic demos, a complete template, gallery/headless rendering guidance. | Import/port Apache-licensed recipes and components with NOTICE; wrap as selectable templates. Audit bundled media rights separately. | Apache-2.0; individual media assets/attributions require separate review. |
...
| [ArcReel](https://github.com/ArcReel/ArcReel) | `d0d7c74` | Novel/script/product inputs; assets and continuity; human gates/version rollback; multi-provider text/image/video/TTS; three video modes; queues/rate limits; cost tracking; final/Jianying exports; archive/import/export; external agent API; auth and production DB. | Clean-room behavioral parity. Do not copy AGPL service/UI code unless the entire distribution decision changes. | AGPL-3.0 plus NOTICE/additional terms. |
...
## Implemented evidence (2026-08-09)
...
- `backend/app/schema/production.py`: mandatory four-category asset catalog, ordered five-view contract, exact nine-panel board, and H3 reference limits.
...
- `backend/app/platform/` and `backend/app/api/{platform,element,user,billing}_api.py`: PostgreSQL users, all 66 globally switchable slash-command abilities, four-class element library, configured development admin bootstrap with a production default-password guard, memberships, orders, append-only ledgers, and signed idempotent callbacks.
...
- Live acceptance: PostgreSQL 16 contains 1 bootstrap administrator, 66 capability records, and 3 membership plans; Playwright verifies login, forced user center, element routing, and capability expansion with zero browser console errors.
